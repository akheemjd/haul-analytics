...
:root{--text:#15171a;--muted:#6b7280;--light:#eaecf0;--bg:#f8f9fa;--card:#fff;--green:#16a34a;--amber:#d97706;--red:#dc2626;--radius:6px;--shadow:0 1px 3px rgba(0,0,0,.04),0 1px 2px rgba(0,0,0,.06)}
body{background:#f5f5f5;color:var(--text);font-family:'Fira Sans',-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;font-size:14px;line-height:1.5;padding:0;max-width:100%}
.banner{background:#fff;border-bottom:1px solid var(--light);padding:0 24px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:1000;height:100px;box-shadow:0 1px 2px rgba(0,0,0,.03)}
...
.main{padding:20px 18px 40px;max-width:1440px;margin:0 auto}
.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(380px,1fr));gap:16px}
.grid-2{grid-template-columns:repeat(2,1fr)}
.card{background:#fff;border:1px solid var(--light);border-radius:var(--radius);padding:20px;scroll-margin-top:70px;box-shadow:var(--shadow)}
.card.full{grid-column:1/-1}
.card-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;padding-bottom:12px;border-bottom:1px solid var(--light);gap:10px}
.card-header h2{font-size:14px;font-weight:700;color:var(--text);font-family:'Fira Mono',monospace;letter-spacing:-.01em}
.card-body{min-height:30px}
.pill{font-size:8px;padding:3px 8px;border-radius:10px;font-weight:600;text-transform:uppercase;letter-spacing:.04em}
...
.ind{background:var(--bg);border-radius:var(--radius);padding:14px 16px;display:flex;flex-direction:column;gap:4px}
...
.diesel-table td{padding:7px 10px;border-bottom:1px solid var(--light)}
.diesel-table .price{font-weight:700;font-family:'Fira Mono',monospace;text-align:right;position:relative}
.fuel-bar{position:absolute;left:0;top:0;height:100%;opacity:.06;border-radius:0 2px 2px 0;pointer-events:none}
.mwrap{display:flex;gap:0;height:380px;overflow:hidden;border-radius:var(--radius)}
.mmap{flex:1;height:380px;min-height:380px;width:100%;background:#f0f0f0;border-radius:var(--radius) 0 0 var(--radius)}.mlist{width:300px;min-width:200px;height:380px;overflow-y:auto;flex-shrink:0;font-size:12px}
.mitem{padding:8px 10px;border-bottom:1px solid var(--light);cursor:pointer;transition:background .1s}
.mitem:hover{background:var(--bg)}
.mhwy{font-weight:600;color:var(--text)}.mdesc{color:var(--muted);font-size:10px;margin-top:1px}
select,input[type=number]{border-radius:4px;transition:border-color .15s}
select:focus,input:focus{outline:none;border-color:var(--text)!important}
@media(max-width:700px){body{font-size:15px}.main{padding:10px 8px 30px}.grid,.grid-2{grid-template-columns:1fr;gap:12px}.card{padding:14px}.inds{grid-template-columns:1fr}.card-header h2{font-size:13px}.mwrap{flex-direction:column;height:auto!important}.mmap{height:260px!important;border-radius:var(--radius) var(--radius) 0 0}.mlist{width:100%;height:220px!important}}"""
